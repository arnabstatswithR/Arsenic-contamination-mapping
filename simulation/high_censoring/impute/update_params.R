...
latent.update <- function(ns, np, Y, X, B.mat, cormat.inv, chol.Sigma, r){
  latent.cov.part1.inv <- diag(ns) / (1- r) +  cormat.inv / r
  latent.cov.part1 <- chol2inv(chol(latent.cov.part1.inv))
...
censored.update <- function(censored.locs, cutoff.Y, Y, X, B.mat, latent, Sigma, r){
  Sigma.1.1 <- Sigma[1, 1]
  Sigma.rest.1 <- Sigma[-1, 1]
  Sigma.1.rest <- Sigma[1, -1]
  Sigma.rest.rest <- Sigma[-1, -1]
  Sigma.rest.rest.inv <- chol2inv(chol(Sigma.rest.rest))
  
  mu.censored <- X[censored.locs, ] %*% B.mat + latent[censored.locs, ]
  
  mean.censored <- mu.censored[ , 1] + 
    (Y[censored.locs, -1] - mu.censored[ , -1]) %*% Sigma.rest.rest.inv %*% Sigma.1.rest
  var.censored <- (1 - r) * (Sigma.1.1 - sum(Sigma.1.rest * c(Sigma.rest.rest.inv %*% Sigma.rest.1)))
  
  excess <- cutoff.Y - mean.censored
  runif.probs <- pnorm(excess / sqrt(var.censored)) * runif(length(censored.locs))
  Y[censored.locs, 1] <- mean.censored + sqrt(var.censored) * qnorm(runif.probs)
  Y}
...
# t(matrix(c(t(kronecker(chol(cormat.predict), chol.Sigma)) %*% rnorm(ns.pred * np)), np, ns.pred))
# 
